...
class ReminderManager:
    ...
    def check_and_notify(
        self, app_id: int, game_name: str, progress: float, current_time: float
        ...
        with self._lock:
            ...
            if new_level.value <= state.current_level.value:
    ...
    def _get_or_create_state(self, app_id: int, game_name: str) -> ReminderState:
        ...
        if app_id not in self._states:
            self._states[app_id] = ReminderState(app_id=app_id, game_name=game_name)
    ...
    def _should_remind(
        self, state: ReminderState, level: ReminderLevel, current_time: float
    ) -> bool:
        """判断是否应该发送提醒"""
    ...
    def _send_reminder(
        ...
        self._notifier.escalate_notify(title=title, message=message, level=level.value)
